...
测试使用独立 AppId `ShuiMan.Installer.NativeSmoke.8BB2BDE2`、`build/windows/installer-native-smoke/installed` 目录、`ShuiMan Native Installer Test` 快捷方式和 `.shuiman-native-install-test` 扩展。本轮安排为静默安装 0.7.1、升级测试标签至 0.7.2、卸载，并验证已安装应用条目、组件、可选关联和数据保留。两个测试包都使用当前 0.7.1 程序文件；0.7.2 仅为升级测试标签，不是应用发行版本。测试不会启动程序或访问真实用户书库。日志和校验结果保存在同级测试目录；保留的“书籍”是脚本生成的文本占位文件。

若有人使用了隔离目录中的测试应用并更改测试书库，脚本会拒绝覆盖该书库。

历史记录：2026-09-13 使用 **0.7.0 原生应用 publish 文件**完成安装器 **23 项检查，0 项失败**，实际执行安装、覆盖升级和卸载。检查包含安装目录无浏览器组件、自带 .NET/PDF 运行文件、Windows 应用登记、可选快捷方式与文件关联、旧脚本安装升级，以及升级和卸载后的原漫画与书库保留。当时的 0.7.1 只是覆盖升级测试标签。本轮已使用实际 **0.7.1 原生应用 publish 文件**重新编译并执行独立安装、0.7.2 测试标签升级和卸载，**23 项检查通过，0 项失败**；结果来自本轮实际执行。
