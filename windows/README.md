...
- 打开书籍或跳到新位置时，先等待当前页及影响布局的相关邻域完成方向与接缝分析，再首次绘制正文，无需点击“应用识别”。有效缓存可直接复用；跳页会动态提高新位置的分析优先级，不必等旧位置扫完整本。随后继续向后识别并补齐其他页面，底部显示进度。结果缓存到 `analysis/`，再次打开续扫未完成页面；阅读设置中可重新分析整本。
- 与 macOS 相同，默认启用积极识别相邻跨页，可关闭以使用更严格的配对判断；普通“双页”布局与智能双图配对分别控制。
- “修正”提供旋转、与下一页配对、交换左右、解除配对、接缝滑块和恢复自动识别。接缝的自动偏移与缩放默认关闭，可在阅读设置中主动开启并按书籍保存；已有手工接缝微调始终保留。手工确认的双图物理左右顺序不随阅读方向改变。
- 阅读时按 `↑` 将当前页顺时针旋转 90°，连续按四次回到原方向；保存为手动修正，优先于 OCR 和出版物方向标记。长按不连续旋转，输入页码、选择下拉项或打开修正菜单时保留控件原有按键操作。
...
智能跨页采用接缝证据，并支持有界的偏移和缩放校正。自动接缝调节独立于智能配对、积极识别开关；新书与未保存该偏好的旧书默认关闭，主动开启过的设置在重开时保留。图像缩小使用面积抗混叠，接缝轮廓使用轻量平滑，减少漫画网点干扰；严格与积极模式保留各自原有的接受阈值。自动方向识别依赖 Windows 可用的 OCR 语言及页面文字线索；竖排文字和低置信度内容不强行旋转，可手动修正。
